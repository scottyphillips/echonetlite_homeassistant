    ...
    for entity in hass.data[DOMAIN][config.entry_id]:
        ...
        for op_code in list(set(entity["instance"]["setmap"]) - _non_setup):
            ...
            if switch_conf := op_code_dict.get(TYPE_NUMBER, {}).get(TYPE_SWITCH):
                ...
                entities.append(EchonetSwitch(connector, config, op_code, combined_conf))
        ...
        if not is_hvac_or_light and not set_enl_status and ENL_STATUS in entity["instance"]["setmap"]:
            entities.append(
                EchonetSwitch(
                    connector, config, ENL_STATUS,
...
class EchonetSwitch(CoordinatorEntity, SwitchEntity):
    ...
    def __init__(self, connector, config, code, options):
        ...
        if isinstance(epc_function_data, list):
            ...
            self._options.update({
                CONF_SERVICE_DATA: {DATA_STATE_ON: data_keys[0], DATA_STATE_OFF: data_keys[1]},
        ...
        base_id = connector._uidi or f"{connector._uid}-{connector._eojgc}-{connector._eojcc}-{connector._eojci}"
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        if await self._connector._instance.setMessage(self._code, self._options[CONF_SERVICE_DATA][DATA_STATE_ON]):
            self._connector._update_data[self._code] = self._options[CONF_SERVICE_DATA][DATA_STATE_ON]
            self.async_write_ha_state()

    async def async_turn_off(self, **kwargs) -> None:
        """Turn the switch off."""
        if await self._connector._instance.setMessage(self._code, self._options[CONF_SERVICE_DATA][DATA_STATE_OFF]):
            self._connector._update_data[self._code] = self._options[CONF_SERVICE_DATA][DATA_STATE_OFF]
            self.async_write_ha_state()
