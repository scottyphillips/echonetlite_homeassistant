...
async def async_setup_entry(hass, config_entry, async_add_devices):
    ...
    for entity in hass.data[DOMAIN][config_entry.entry_id]:
        ...
        if (eojgc == 0x02 and eojcc in (0x90, 0x91, 0xA3)) or (
            ...
            entities.append(
                EchonetLight(entity["echonetlite"], config_entry, custom_options)
            )
...
class EchonetLight(CoordinatorEntity, LightEntity):
    ...
    def __init__(self, connector, config, custom_options):
        ...
        self._light_color_level_max = getattr(
            connector._instance, "getLightColorLevelMax", lambda: 100
        )()

    @property
    def is_on(self):
        return (
            self._connector._update_data.get(self._custom_options[ENL_STATUS])
            == DATA_STATE_ON
        )

    @property
    def brightness(self):
        brightness = self._connector._update_data.get(
